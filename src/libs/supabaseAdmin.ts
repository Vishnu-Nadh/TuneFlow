...
const upsertProductRecord = async (product: Stripe.Product) => {
  ...
  console.log("producted updated", product.id);
};
...
const createOrRetriveCustomer = async ({ email, uuid }: { email: string; uuid: string }) => {
  ...
  if (error || !data.stripe_customer_id) {
    ...
    console.log("new customer inserted ", { uuid });
    return customer.id;
  }
...
export { upsertPriceRecord, upsertProductRecord, createOrRetriveCustomer, manageSubscriptionStatusChange };
